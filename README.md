...
| [CVE-2026-64469](cves/cve-2026-64469-frostwalk/README.md) — Frostwalk | a sibling of Frostbind, in the same function: `binder_free_transaction()`'s unlocked read races [`binder_thread_release()`](https://android.googlesource.com/kernel/common/+/refs/heads/android14-6.1/drivers/android/binder.c) walking a *different* thread's `transaction_stack` | hunt; free and walk both confirmed reachable, 4/4 on hardware, with no root dependency in the mechanism — reaches an app process the way [Shizuku](https://github.com/RikkaApps/Shizuku) does; a first reclaim attempt (flood + pin, no verify-read) went 4/4 MISS, device survived — the write's actual target is still not distinguished |
...
- [tools/](tools/) — [hwbp](tools/hwbp/README.md), an on-device instruction counter; [pixel-image](tools/pixel-image/README.md), which extracts offsets from a published OTA; and [cve-monitor](tools/cve-monitor/README.md), a ranked feed of new Linux kernel CVEs.
