...
// .factory-images/tokay-cp2a.260705.006/Image:
//
...
//     layout. CONFIG_RANDSTRUCT is not set in this build's embedded
//     IKCONFIG.
//
// blazer/mustang/rango are android15-6.6 targets. Their struct offsets are
// not valid here; see the DIVERGENCE notes below. Anything
// tagged "arch constant" is the one category that is neither a symbol nor a
// struct member, and is called out individually.
//
// STATUS: complete, self-consistent, never run on hardware.
...
// The reclaim knobs stay at common.h's defaults
// (RECLAIM_KEEP_PCP_SHAPING 0, RECLAIM_SPRAY_AFTER_LEAK_FREE 0,
// SKB_RECLAIM_SENDS 4): raising them does not help on this device and costs
// the slide route attempts.

// Left at common.h's 5. MM_PARTIALS also sizes the spray: 12 would take it
// from 192 children to 416, each another mm_struct allocation and free
// competing for the page the reclaim is trying to win.

// CONFIG_SLUB_CPU_PARTIAL is set on this build, and on a reclaim miss the
// emptied target slab is handed back out as an mm_struct rather than reaching
// the page allocator. It shows up as fake_lock + 0x10 reading
// mm_struct.data_vm (BTF offset 0xe0) — observed as 0x91b on two boots and
// 0x91a on a third.
...
// macros therefore alias onto the same bytes here: util.c and slide.c write
// FAKE_WAITER_PRIO to both prio macros and 0 to both deadline macros, so the
// aliased writes are idempotent. Field order also differs — task/lock/wake_state precede
...
//
// The object size is not sizeof(struct mm_struct) (960 in this build's BTF):
// proc_caches_init() sizes the cache as
...
// This device boots with cgroup.memory=nokmem (see /proc/cmdline), so
// /proc/slabinfo lists only kmalloc-* and kmalloc-rcl-* and no kmalloc-cg-*.
// nokmem makes new_kmalloc_cache() alias the CGROUP row onto NORMAL rather
// than remove it, so the enum and the row count are unchanged, row 1 is a
// valid cache pointer, and GFP_KERNEL_ACCOUNT pipe_buffer arrays land in that
// same cache.
...
// ── main route transport (61/) ──────────────────────────────────────────
// Use the TCP route, not pselect. The pselect main route fails here: it hits
// the rb_leftmost misalignment 61/common.h documents for pselect on 6.1
// (a panic in rt_mutex_top_waiter() reading lock->waiters.rb_leftmost), which
// is why common.h tells 6.1 targets to override this to 1.
