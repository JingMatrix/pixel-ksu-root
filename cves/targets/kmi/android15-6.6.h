...
#endif
/* The fops slot inside the misc device record, not the record itself. */
...
#endif
/* The enforcing flag is the first member of the policy state record. */
...
/* ── Slide references ──
 * Symbols a text-base recovery reads or reaches through. SLIDE_TRACE_MARK_IP_OFF
 * is the code address a trace-marker write stores in its own record: the only
 * value a write-free text-base recovery needs. */
...
#endif
/* The data pointer of the boot-id sysctl entry, not the buffer it names. */
