...
static int lib_binder_transact(int fd, uint32_t cmd, uint32_t handle, int oneway,
...
int lib_binder_send(int fd, uint32_t handle, int oneway,
		    ...
		    void *rbuf, size_t rlen, size_t *consumed)
{
	return lib_binder_transact(fd, BC_TRANSACTION, handle, oneway,
...
int lib_binder_reply(int fd, const void *data, size_t dlen,
		     const void *offsets, size_t olen)
{
	return lib_binder_transact(fd, BC_REPLY, 0, 0, data, dlen,
