...
|---|---|---|
| [`kprobe.sh`](kprobe.sh) | was this function reached, through a [kprobe](https://docs.kernel.org/trace/kprobes.html), with what arguments, and did it return; what is at this address | the device, as root |
...
| [`DeriveAmclientProtocol.java`](DeriveAmclientProtocol.java) | what are [`amclient.h`](../trigger/amclient.h)'s hidden-API transaction codes and `writeInterfaceToken` byte header on *this* device — Java, not shell, run via `app_process`, since only a live `Parcel`/reflection answers it | the device, plain shell |
...
[`kprobe.sh`](kprobe.sh) removes every probe and restores the tracing state on the way out,
