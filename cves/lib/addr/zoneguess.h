 ...
 * occupies. A device constant: nothing shell-readable reports populated frame
 * ranges. Narrower than the whole span on purpose -- most of `spanned` is
 * unpopulated address space a draw there can never hit.
 *
 * Measure it in the conditions the consumer sprays in. The band a spray occupies
 * is a function of what the free lists looked like when it ran, so a histogram
 * taken from a standalone process on a settled device describes a different
 * spray and names a different window. The figure that counts is the consumer's
 * own end-to-end hit rate.
 */
