...
| `mmguard.h` | staying usable once this process's own mappings describe something the kernel must not walk: make every page resident, close the mappings to other readers, and read addresses that may refuse to be read without the fault reaching the process |
...
| `interval.h` | how far apart two of those events are, as a spread rather than a number |
