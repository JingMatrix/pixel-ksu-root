...
def emit_header(rows, target, image, nsym, kbase):
...
def main():
    ...
    rows = derive(img, syms, offset_rules.load_maps(), kbase, seed)
    if a.compare:
        return 2 if emit_compare(rows, load_committed(a.compare)) else 0
    emit_header(rows, a.target, a.image, len(syms), kbase)
