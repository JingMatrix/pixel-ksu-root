...
# PAGETRACE_SKB_LEN selects the AF_UNIX payload length, which is what decides
# whether the skb vehicle lands in kmalloc-8k or goes through kmalloc_large to a
# whole page-allocator block. Env does not survive `adb shell`, so pass it
# explicitly.
ENVV=""
[ -n "${PAGETRACE_SKB_LEN:-}" ] && ENVV="PAGETRACE_SKB_LEN=$PAGETRACE_SKB_LEN"
if [ "${PAGETRACE_AS_ROOT:-0}" = 1 ]; then
  adb shell su -c "$ENVV $DEV $*"
else
  adb shell env $ENVV "$DEV" "$@"
fi
