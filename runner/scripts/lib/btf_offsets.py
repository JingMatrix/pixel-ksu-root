...
class Btf:
    def __init__(self, src):
        ...
        self.types = [None]
        self.by_name = {}
        # enumerator name -> value, across every enum in the blob. Flag bit
        # positions and cache-row indices are enumerators, and a transcription
        # of one is exactly as easy to get wrong as a struct offset.
        self.enumerators = {}
        ...
        while off < end:
            ...
            if kind in (KIND_STRUCT, KIND_UNION):
                ...
                for i in range(vlen):
                    e_name, e_val = struct.unpack_from("<Ii", blob, off + i * 8)
                    en = name(e_name)
                    if en:
                        self.enumerators.setdefault(en, e_val)
                off += vlen * 8
            else:
    ...
    def enumerator(self, name):
        """Value of an enumerator by name, or raise ValueError."""
        if name not in self.enumerators:
            raise ValueError(f"no such enumerator: {name}")
