...
def main():
    ...
    # Every definition is guarded, and the build description is included first,
    # so this file is a third layer over (interface, build): it adds what one
    # chain needs and never contradicts what the target already states.
    def emit(macro, value, width=32):
        L.append(f"#ifndef {macro}")
        L.append(f"#define {macro:<{width}} {value}")
        L.append("#endif")
    ...
    emit("KIMAGE_TEXT_BASE", f"UINT64_C({KIMAGE_TEXT_BASE:#018x})")
    emit("LINK_IMAGE_BASE",  f"UINT64_C({KIMAGE_TEXT_BASE:#018x})")
    emit("PAGE_OFFSET_VA",   f"UINT64_C({PAGE_OFFSET:#018x})")
    L += ["",
        "/* ---- kernel symbols (link-time addresses, keyed to this build) ---- */",
    ]
    for macro, sym in SYMBOLS:
        if sym in syms:
            emit(macro, f"UINT64_C({KIMAGE_TEXT_BASE + syms[sym] - text:#018x})")
        else:
    ...
    for macro, sym in DIRECT:
        if sym in syms:
            emit(macro, f"UINT64_C({PAGE_OFFSET + syms[sym] - text:#018x})")
        else:
            unresolved.append(f"{macro} ({sym})")
    ...
    if ctl_sz and "random_table" in syms:
        ...
        if idx is None:
            ...
            emit("DIRECT_BOOTID_PARENT", f"UINT64_C({PAGE_OFFSET + rt_rt - text:#018x})")
            emit("DIRECT_BOOTID_DATA", f"UINT64_C({PAGE_OFFSET + rt_rt + data_off - text:#018x})")
    ...
    for macro, sym, sname, field in CYCLES:
        ...
        emit("LINK_" + macro, f"UINT64_C({KIMAGE_TEXT_BASE + runtime - text:#018x})", 27)
        emit("DIRECT_" + macro, f"UINT64_C({PAGE_OFFSET + runtime - text:#018x})", 25)
    ...
    for macro, sname, field in STRUCTS:
        ...
        try:
            ...
            continue
        emit(macro, f"{(extra if off is None else off):#06x}")

    L += ["", "#endif /* TARGET_FACTS_CVE64560_H */"]
    text_out = "\n".join(L) + "\n"
    if a.out:
