...
# Targets come from the manifest, not from the directory listing: a build whose
# header is composed from its manifest row has no directory of its own, and one
# that has a directory may have it for something other than a target header.
targets=()
while IFS= read -r t; do targets+=("$t"); done < <(python3 - ../data/targets.json <<'PYEOF'
import json, sys
seen = []
for d in json.load(open(sys.argv[1]))["devices"]:
    t = "%s-%s" % (d["codename"], d["build"])
    if t not in seen:
        seen.append(t)
print("\n".join(sorted(seen)))
PYEOF
