...
# nothing persists — so most of this is gone the moment the device reboots.
#
# Most, not all. /proc/slabinfo and /sys/fs/pstore/* are readable by plain adb
# shell, and are collected without su, so a harvest on a never-rooted phone
# still returns the slab layout and the previous boot's console. Root is needed
# for /proc/kallsyms (with kptr_restrict lifted), BTF, config.gz, /proc/iomem
# and dmesg.
#
...
harvest_one() {
  ...
  # strategy (posix_timers_cache is dedicated, not kmalloc). Readable by shell.
  "${A[@]}" shell 'cat /proc/slabinfo' 2>/dev/null | tr -d '\r' > "$out/slabinfo.txt"
  ...
  # pstore needs no root. /sys/fs/pstore is dr-xr-x--- system:log and shell is
  # in group log; sepolicy grants shell read on pstorefs:file but not dir read,
  # so `ls` is denied and `cat <known name>` is not. Naming the three files
  # explicitly is therefore not a shortcut, it is the only way in.
  for f in console-ramoops-0 pmsg-ramoops-0 dmesg-ramoops-0; do
    "${A[@]}" shell "cat /sys/fs/pstore/$f" 2>/dev/null | tr -d '\r' > "$out/$f" || true
