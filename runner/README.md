...
| [`entry.zombietick@6.1`](stages/entry.zombietick@6.1/stage.toml) | entry | CVE-2026-64560 | android14-6.1 | static-exe | [no](stages/entry.zombietick@6.1/stage.toml#L34) |
...
[`recipes/zombietick.toml`](recipes/zombietick.toml) is one entry and no non-entry
stages ([`stages = []`](recipes/zombietick.toml#L33)). That is what makes a
`--recipe zombietick` run a *hunt* — with no stage providing `CAP_SU` there is no
...
[`../cves/cve-2026-43499-ghostlock/common.h`](../cves/cve-2026-43499-ghostlock/common.h#L201)
and implemented in
[`util_addr.c`](../cves/cve-2026-43499-ghostlock/util_addr.c):
...
([common.h](../cves/cve-2026-43499-ghostlock/common.h#L208)). The conversions
live in [util.c](../cves/cve-2026-43499-ghostlock/util_addr.c):

- [`k_direct(kimage_t)`](../cves/cve-2026-43499-ghostlock/util_addr.c#L305) —
  total, KASLR-free; the physmap alias.
- [`k_run(const struct kslide *, kimage_t)`](../cves/cve-2026-43499-ghostlock/util_addr.c#L315)
  — for an address you are about to touch; traps on an unknown slide.
- [`k_direct_to_run(...)`](../cves/cve-2026-43499-ghostlock/util_addr.c#L336) — the
  inverse pair.
- [`k_image_raw(kimage_t)`](../cves/cve-2026-43499-ghostlock/util_addr.c#L340) — a
...
[../cves/lib/kaslr/README.md](../cves/lib/kaslr/README.md).
...
silent predicate,
[`is_direct_ptr`](../cves/cve-2026-43499-ghostlock/util_addr.c#L902), whose failure
returns a zero indistinguishable from a real read (it gates every read and write
in [pipe.c](../cves/cve-2026-43499-ghostlock/pipe.c#L354)). Typing the
...
[`put_fake_fops_table()`](../cves/cve-2026-43499-ghostlock/util_addr.c#L388) fills
the fake fops slots with image addresses while the slide is still
`SLIDE_UNKNOWN`, and
[`refresh_fake_fops_text()`](../cves/cve-2026-43499-ghostlock/fops.c#L638)
rewrites them the instant it lands. Hence two functions:
[`k_run` traps on `SLIDE_UNKNOWN`](../cves/cve-2026-43499-ghostlock/util_addr.c#L316),
...
([common.h](../cves/cve-2026-43499-ghostlock/common.h#L195)).
...
([entry.zombietick@6.1](stages/entry.zombietick@6.1/stage.toml#L113)), so a
`--recipe zombietick` hunt leaks afresh on every shot.
...
([entry.zombietick@6.1](stages/entry.zombietick@6.1/stage.toml#L97)); `"runner"`
...
[`TARGET_PROFILE_GATE_FAIL`](stages/entry.zombietick@6.1/stage.toml#L140). An
...
[is populated](stages/entry.zombietick@6.1/stage.toml#L188) because its binary is
...
([entry.zombietick@6.1](stages/entry.zombietick@6.1/stage.toml#L81)). The resolver
...
3. Composition into shared `groom`/`bridge`/`rw`/`effect`/`handoff` stages, so it
   ...
   [`composed`](stages/entry.zombietick@6.1/stage.toml#L34).
